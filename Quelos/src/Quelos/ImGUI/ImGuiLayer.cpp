...
// TODO: Clean up file and abstract bgfx calls
...
namespace Quelos {
	...
	///
	inline bool checkAvailTransientBuffers(const uint32_t numVertices, const bgfx::VertexLayout& layout, const uint32_t numIndices) {
	...
	struct ImGuiState {
		...
		void Create(const float fontSize) {
			IMGUI_CHECKVERSION();
		...
		void BeginFrame(const bgfx::ViewId viewId) {
			ViewId = viewId;
	...
	static ImGuiState s_ImGuiState;
