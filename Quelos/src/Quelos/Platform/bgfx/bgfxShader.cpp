...
namespace Quelos {
    ...
    bgfxShader::bgfxShader(const std::string& filePathVertex, const std::string& filePathFragment) {
    ...
    void bgfxShader::Submit(const uint32_t view) {
        bgfx::submit(view, m_ProgramHandle);
    }

    bgfxShader::~bgfxShader() { bgfx::destroy(m_ProgramHandle); }
}
