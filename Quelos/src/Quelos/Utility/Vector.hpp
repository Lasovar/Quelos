...
// INITIAL VERSION GENERATED BY CLAUD, reviewed/edited by me (@lasovar)
...
namespace Quelos {
    ...
    public:
            ...
            : m_Resource(memoryResource)
        {
            ...
            if (m_Resource == other.m_Resource) {
                ...
                memory::relocate_range(m_Data, other.m_Data, other.m_Size, allocator_type(memoryResource));
        ...
        VectorT(VectorT&& other, const allocator_type& allocator) noexcept
            : VectorT(std::move(other), allocator.resource()) { }

        VectorT(VectorT&& other, const AllocatorType allocatorType) noexcept
            : VectorT(std::move(other), GetAllocator(allocatorType)) { }
        ...
        [[nodiscard]] VectorT clone(std::pmr::memory_resource* resource) const {
            ...
            memory::uninitialized_copy_range(result.m_Data, m_Data, m_Size, allocator_type(result.m_Resource));
            result.m_Size = m_Size;

            return result;
        }

        [[nodiscard]] VectorT clone(const allocator_type allocator) const {
            return clone(allocator.resource());
        }
    ...
    private:
        ...
        [[nodiscard]] constexpr T* AllocateStorage(size_type capacity) const {
            ...
            void* raw = m_Resource->allocate(memory::byte_count<T>(capacity), alignof(T));
            return static_cast<T*>(raw);
        }

        constexpr void DeallocateStorage(T* ptr, size_type capacity) noexcept {
            if (ptr != nullptr) {
                m_Resource->deallocate(ptr, memory::byte_count<T>(capacity), alignof(T));
            }
        }
