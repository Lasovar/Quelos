...
namespace Quelos {
    ...
    void RegisterComponent(
        flecs::world& world,
        Map<RuntimeID, InspectorComponent>& componentMap
    ) {
        ...
        componentMap[world.id<TComponent>()] = {
            inspectorSerialize, setFieldSerialize, ComponentRegistry::TypeName<TComponent>()
        };
    }
        ...
        if (ImGui::Begin("Entity Inspector")) {
            if (m_SelectedEntity.IsAlive()) {
                m_SelectedEntity.GetID().each([this](const flecs::id runtimeId) {
                    ...
                    ImGui::Text(std::string(inspectorComponent.ComponentName).c_str());
